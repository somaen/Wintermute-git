...
void ConfigManager::loadDefaultConfigFile() {
	...
	if (stream) {
		...
		warning("Default configuration file missing, creating a new one");
...
void ConfigManager::loadConfigFile(const String &filename) {
	...
	if (!cfg_file.open(node)) {
		warning("Creating configuration file: %s", filename.c_str());
	} else {
		warning("Using configuration file: %s", _filename.c_str());
