...
JPEGDecoder::JPEGDecoder() : ImageDecoder(), _stream(NULL), _w(0), _h(0),
	_numComp(0), _components(NULL), _numScanComp(0), _scanComp(NULL),
	_currentComp(NULL), _rgbSurface(0) {
...
bool JPEGDecoder::readJFIF() {
	...
	if (majorVersion != 1 || (minorVersion != 1 && minorVersion != 2))
		warning("JPEGDecoder::readJFIF() Non-v1.1/1.2 JPEGs may not be handled correctly");
